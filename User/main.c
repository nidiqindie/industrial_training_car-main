...
void programme_2_to_zancun();
int programme_2_to_temporary_storage_flag = 0;
int main(void)
{
    programme_2();
}
...
void programme_2()
{
    ...
    TurnLeft(150, 150, 90);
    delay_ms(2000);
    move_right(speed_all, 20, 0.35);
    delay_ms(500);
    yaw_run(180, smill_calibrations);

    catch_p();
    move_forward(speed_all, acc_all, 0.45);
    delay_ms(500);
    // 加工物料
    weitiao(0);
    now = ring_color == red_num ? red_num : (ring_color == blue_num ? blue_num : green_num);
    ...
    // 第一圈粗加工到暂存区
    programme_2_to_temporary_storage();
    ...
    */
    ...
    // 在这里写放置函数
    // 微调
    move_forward(speed_all, acc_all, 0.53);
    delay_ms(500);
    catch_p();
    weitiao(0);
    now = ring_color == red_num ? red_num : (ring_color == blue_num ? blue_num : green_num);
...
void programme_2_to_temporary_storage()
{
    go_to_target(1, 0);
    move_left(speed_all, 20, 0.3);
    delay_ms(500);
    ...
    TurnRight(150, 150, 90);
    delay_ms(2000);
    move_backward(speed_all, acc_all, 2.57);
    delay_ms(3000);
    move_right(speed_all, 20, 0.1);
    delay_ms(200);
    now = 3;
    programme_2_to_temporary_storage_flag++;
    int temp = 0;
    if (programme_2_to_temporary_storage_flag == 1) {
        temp = 0;
    }else
    {
        temp = 2;
